...
def make_data(true_params, consts, county=-1, return_all=False):
    ...
    # Ratio of H -> D, vs H -> R
    fatality_H = simple_function(params['fatality_H']) * np.ones(num_counties)
    ...
    for i in counties:
        ...
        for t in range(T - 1):
            ...
            R.append(R[-1] + (1 - fatality_I[i]) * rho_IR[i] * I[-2] + (1 - fatality_H[i]) * rho_HR[i] * H[-2])
            # Dead compartment
            D.append(D[-1] + fatality_I[i] * rho_IR[i] * I[-2] + fatality_H[i] * rho_HR[i] * H[-2])
        if return_all:
            X_data.append(E)
            X_data.append(I)
            X_data.append(H)
...
def simple_function(w):
    return shared.max_rate * sigmoid(np.array(w))
...
def sig_function(x, w, b=0):
    ...
    num_vals = shapes[-1]
    if len(shapes) == 3:
        return max_rate * sigmoid(
            np.sum(x * np.reshape(w, (num_counties, 1, num_vals)), axis=2) + np.reshape(b, (num_counties, 1)))
    else:
        return max_rate * sigmoid(np.sum(x * np.array(w), axis=1) + np.array(b))
