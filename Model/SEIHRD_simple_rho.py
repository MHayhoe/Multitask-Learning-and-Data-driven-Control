...
def make_data(true_params, consts, county=-1, return_all=False):
    ...
    # I -> D
    rho_ID = simple_function(params['rho_ID_coeffs']) * np.ones(num_counties)
    # H -> D
    rho_HD = simple_function(params['rho_HD_coeffs']) * np.ones(num_counties)
...
def sig_function(x,w,b=0):
    ...
    num_vals = shapes[-1]
    if len(shapes) == 3:
        return max_rate * sigmoid(np.sum(x * np.reshape(w,(num_counties,1,num_vals)), axis=2) + np.reshape(b,(num_counties,1)))
    else:
